...
cache_check_replay_as_dir(const hs_cache_dir_descriptor_t *desc)
{
  int ret = 0;
  time_t elapsed;
  char *desc_without_sig;

  tor_assert(desc);

  desc_without_sig = hs_desc_remove_sig(desc->encoded_desc);
  if (BUG(desc_without_sig == NULL)) {
